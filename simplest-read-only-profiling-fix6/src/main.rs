...
// ============================================================================
// Helper Functions - CRITICAL OPTIMIZATIONS
// ============================================================================
...
// ============================================================================
// Core Data Reading Function - PROPERLY OPTIMIZED FOR 64 THREADS
// ============================================================================
...
fn read_timstof_data(d_folder: &Path) -> Result<TimsTOFRawData, Box<dyn Error>> {
    ...
    // Step 3: DIRECT PARALLEL PROCESSING - NO PRODUCER/CONSUMER!
    // This is what actually works - let all 64 threads read and process directly
    let frame_proc_start = Instant::now();
    println!("  Processing {} frames with 64 threads (direct parallel)...", n_frames);
    
    // Process frames in smaller chunks to improve work distribution
    // With 64 threads and 53168 frames, each thread gets ~830 frames
    // But we'll use dynamic scheduling via rayon
    let splits: Vec<FrameSplit> = (0..n_frames).into_par_iter()
        .map(|idx| {
            // Each thread reads and processes its own frames
            ...
            match frame.ms_level {
                MSLevel::MS1 => {
                    ...
                    // Build scan lookup once per frame
    ...
    // Step 4: Merge MS1 data - OPTIMIZED FROM FIX1
    ...
    // Step 5: Merge MS2 data - EXACT COPY FROM FIX1 (PROVEN TO WORK)
    ...
    // Pre-calculate total MS2 size per window to avoid reallocation
...
fn main() -> Result<(), Box<dyn Error>> {
    // HARD-CODED: Use exactly 64 threads as requested
    ...
    println!("\n╔══════════════════════════════════════════════════════════════╗");
    println!("║      OPTIMIZED TimsTOF Raw Data Reader - 64 THREADS           ║");
    println!("╚══════════════════════════════════════════════════════════════╝");
    ...
    println!("  Expected MS1: 591487824, Got: {}", raw_data.ms1_data.mz_values.len());
    println!("  Expected MS2 windows: 32, Got: {}", raw_data.ms2_windows.len());
    println!("  Expected MS2 peaks: 2034948969, Got: {}", total_ms2);
    
    if raw_data.ms1_data.mz_values.len() == 591487824 
        && raw_data.ms2_windows.len() == 32 
        && total_ms2 == 2034948969 {
        println!("✓ Output verification PASSED - results match original!");
    } else {
        println!("✗ Output verification FAILED - results differ from original!");
    }

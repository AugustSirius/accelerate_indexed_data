// Complete optimized utils.rs file
...
const CHANNEL_BUFFER_SIZE: usize = 10000;  // Increased from 2000
const BATCH_SIZE: usize = 1000;  // Process frames in batches
const PREFETCH_FRAMES: usize = 100;  // Prefetch frames for better I/O
...
// ============================================================================
// Optimized TimsTOFData with better memory layout
// ============================================================================
...
// Helper structures for optimized read_timstof_data
#[derive(Clone)]
enum ProcessedFrame {
...
impl FrameProcessor {
    ...
    fn process_peaks_vectorized(
        ...
        // Vectorized processing using chunks
        const CHUNK_SIZE: usize = 64;  // Process 64 peaks at a time for SIMD
...
fn estimate_total_peaks_fast(frames: &FrameReader) -> Result<(usize, usize), Box<dyn Error>> {
    // Sample more frames for better estimate
    ...
    // Estimate frame distribution (typically 40% MS1, 60% MS2)
...
// ============================================================================
// ULTRA-FAST read_timstof_data with optimized parallel processing
// ============================================================================
pub fn read_timstof_data(d_folder: &Path) -> Result<TimsTOFRawData, Box<dyn Error>> {
    ...
    // Estimate data size for better pre-allocation
    ...
    println!("Processing frames with optimized pipeline ({} threads)...", get_num_threads());
    let process_start = Instant::now();
    
    // Use unbounded channel for better throughput
    let (sender, receiver) = unbounded();
    let processed_count = Arc::new(AtomicUsize::new(0));
    
    // Pre-allocate data structures with estimated sizes
    let ms1_accumulator = Arc::new(Mutex::new(Vec::with_capacity(n_frames / 10)));
    let ms2_map = Arc::new(DashMap::with_capacity_and_hasher(
        200, 
    ...
    let aggregator_handle = std::thread::spawn(move || {
        let mut local_ms1_buffer = Vec::with_capacity(100);
        let mut local_ms2_buffer: HashMap<(u32, u32), TimsTOFData> = HashMap::with_capacity(100);
        
        while let Ok(frame_data) = receiver.recv() {
            match frame_data {
                ProcessedFrame::MS1(_, data) => {
                    if !data.mz_values.is_empty() {
                        local_ms1_buffer.push(data);
                        
                        // Batch flush to reduce lock contention
                        if local_ms1_buffer.len() >= 50 {
                            let mut global = ms1_acc_clone.lock();
                            global.extend(local_ms1_buffer.drain(..));
                        }
                    }
                }
                ProcessedFrame::MS2(_, pairs) => {
                    for (key, data) in pairs {
                        if !data.mz_values.is_empty() {
                            local_ms2_buffer.entry(key)
                                .or_insert_with(TimsTOFData::new)
                                .merge_from(data);
                        }
                    }
                    
                    // Batch flush MS2 data
                    if local_ms2_buffer.len() >= 50 {
                        for (key, data) in local_ms2_buffer.drain() {
        ...
        if !local_ms1_buffer.is_empty() {
            ms1_acc_clone.lock().extend(local_ms1_buffer);
        }
        for (key, data) in local_ms2_buffer {
            ms2_map_clone.entry(key)
                .or_insert_with(|| Arc::new(Mutex::new(TimsTOFData::new())))
                .lock()
                .merge_from(data);
    ...
    // Process frames in batches for better cache locality
...
// ============================================================================
// OPTIMIZED IndexedTimsTOFData with SIMD sorting
// ============================================================================
...
impl IndexedTimsTOFData {
    ...
    // Optimized parallel build with better memory access patterns
    pub fn from_timstof_data(data: TimsTOFData) -> Self {
        let n_peaks = data.mz_values.len();
        
        const PARALLEL_THRESHOLD: usize = 50_000;  // Increased threshold
    ...
    fn from_timstof_data_sequential(data: TimsTOFData) -> Self {
        ...
        let mut indices: Vec<(usize, u32)> = (0..n_peaks)
            .map(|i| (i, unsafe { (data.mz_values[i] * 1_000_000.0).to_int_unchecked::<u32>() }))
            .collect();
        
        indices.sort_unstable_by_key(|&(_, mz)| mz);
        
        let order: Vec<usize> = indices.into_iter().map(|(i, _)| i).collect();
        
        // Optimized reordering with pre-allocated vectors
        let mut result = Self {
    ...
    fn from_timstof_data_parallel_optimized(data: TimsTOFData) -> Self {
        ...
        // Parallel sorting with better cache usage
        let mut indexed_mz: Vec<(usize, u32)> = (0..n_peaks)
            .into_par_iter()
            .map(|i| (i, unsafe { (data.mz_values[i] * 1_000_000.0).to_int_unchecked::<u32>() }))
            .collect();
        
        // Use parallel unstable sort for speed
        ...
        // Parallel reordering with chunked processing for better cache locality
        ...
        result.rt_values_min.par_chunks_mut(CHUNK_SIZE)
            ...
            .zip(order.par_chunks(CHUNK_SIZE))
            .enumerate()
            .for_each(|(chunk_idx, ((((((rt, mob), mz), int), frame), scan), ord))| {
                let base_idx = chunk_idx * CHUNK_SIZE;
                for (i, &old_idx) in ord.iter().enumerate() {
                    rt[i] = data.rt_values_min[old_idx];
                    mob[i] = data.mobility_values[old_idx];
                    mz[i] = data.mz_values[old_idx];
                    int[i] = data.intensity_values[old_idx];
                    frame[i] = data.frame_indices[old_idx];
                    scan[i] = data.scan_indices[old_idx];
                }
            });
    ...
    // Optimized binary search using SIMD-friendly algorithm
    #[inline(always)]
    fn range_indices(&self, mz_min: f32, mz_max: f32) -> std::ops::Range<usize> {
        // Convert to integers for faster comparison
        ...
        // Custom binary search optimized for our use case
    ...
    pub fn slice_by_mz_range(&self, mz_min: f32, mz_max: f32) -> TimsTOFData {
        ...
        // Use unsafe block for faster copying
        unsafe {
    ...
    pub fn slice_by_mz_im_range(&self, mz_min: f32, mz_max: f32, im_min: f32, im_max: f32) -> TimsTOFData {
        ...
        // Parallel filtering for large ranges
        ...
        // Vectorized copy for filtered indices
        for &i in &indices {
...
// Optimized build_indexed_data with better parallelization
pub fn build_indexed_data(raw_data: TimsTOFRawData) -> Result<(IndexedTimsTOFData, Vec<((f32, f32), IndexedTimsTOFData)>), Box<dyn Error>> {
    ...
    // Process MS1 and MS2 in parallel with optimal thread allocation
    let (ms1_indexed, ms2_indexed_pairs) = rayon::join(
        || {
            println!("  Building MS1 index...");
            IndexedTimsTOFData::from_timstof_data(raw_data.ms1_data)
        },
        || {
            println!("  Building MS2 indices...");
            if ms2_count > 10 {
                // Process MS2 windows in parallel batches
                ...
                // For small number of windows, process sequentially
                raw_data.ms2_windows
                    .into_iter()
                    .map(|((low, high), data)| {
                        let indexed = IndexedTimsTOFData::from_timstof_data(data);
...
// Helper trait for merge operations
pub trait MergeFrom {
...
pub fn prepare_precursor_lib_data(
    ...
    // Get first N unique precursor IDs

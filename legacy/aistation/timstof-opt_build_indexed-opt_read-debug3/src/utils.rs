// Complete optimized utils.rs file with all performance improvements

use std::collections::HashMap;
use ndarray::{Array2, Array3, s, Axis, concatenate};
...
// CPU feature detection for prefetching
...
const AGGREGATOR_THREADS: usize = 8;
const BATCH_SIZE: usize = 1000;
const MS1_CHUNK_SIZE: usize = 100_000;
const MS2_CHUNK_SIZE: usize = 100_000;
const CACHE_LINE_SIZE: usize = 64;

// Library processing constants
...
impl TimsTOFData {
    ...
    pub fn with_capacity(capacity: usize) -> Self {
        let aligned_cap = ((capacity + 63) / 64) * 64;
        Self {
    ...
    pub fn with_capacity_aligned(capacity: usize) -> Self {
        ...
        // Pre-touch pages if large allocation
        if aligned_cap > 100_000 {
            data.rt_values_min.resize(aligned_cap, 0.0);
            data.mobility_values.resize(aligned_cap, 0.0);
            data.mz_values.resize(aligned_cap, 0.0);
            data.intensity_values.resize(aligned_cap, 0);
            data.frame_indices.resize(aligned_cap, 0);
            data.scan_indices.resize(aligned_cap, 0);
            
            data.rt_values_min.clear();
            data.mobility_values.clear();
            data.mz_values.clear();
            data.intensity_values.clear();
            data.frame_indices.clear();
            data.scan_indices.clear();
        }
    ...
    pub unsafe fn append_unchecked(&mut self, other: &mut Self) {
        ...
        self.rt_values_min.reserve_exact(other_len);
        self.mobility_values.reserve_exact(other_len);
        self.mz_values.reserve_exact(other_len);
        self.intensity_values.reserve_exact(other_len);
        self.frame_indices.reserve_exact(other_len);
        self.scan_indices.reserve_exact(other_len);
    ...
    pub fn merge_from(&mut self, other: TimsTOFData) {
    ...
    pub fn merge(data_list: Vec<TimsTOFData>) -> Self {
        ...
        let total_capacity: usize = data_list.iter()
            .map(|d| d.mz_values.len())
            .sum();
        
        let mut merged = TimsTOFData::with_capacity(total_capacity);
        
        for mut data in data_list.into_iter() {
            unsafe { merged.append_unchecked(&mut data); }
        }
...
// ============================================================================
// Optimized peak processing
// ============================================================================
#[inline(always)]
fn process_peaks_vectorized_optimized(
    tof_indices: &[u32],
    intensities: &[u32],
    scan_offsets: &[usize],
    rt_min: f32,
    ...
    let mut data = TimsTOFData::with_capacity_aligned(n_peaks);
    
    const CHUNK_SIZE: usize = 256;
    const PREFETCH_DISTANCE: usize = 4;
    
    let mut scan_idx = 0;
    let mut next_scan_offset = if scan_offsets.len() > 1 { scan_offsets[1] } else { n_peaks };
    
    for chunk_idx in 0..(n_peaks + CHUNK_SIZE - 1) / CHUNK_SIZE {
        let chunk_start = chunk_idx * CHUNK_SIZE;
        let chunk_end = (chunk_start + CHUNK_SIZE).min(n_peaks);
        
        // Prefetch next chunks
        let prefetch_idx = chunk_idx + PREFETCH_DISTANCE;
        let prefetch_start = prefetch_idx * CHUNK_SIZE;
        if prefetch_start < n_peaks {
            prefetch_read(&tof_indices[prefetch_start]);
            prefetch_read(&intensities[prefetch_start]);
        }
        
        for p_idx in chunk_start..chunk_end {
            while p_idx >= next_scan_offset && scan_idx < scan_offsets.len() - 1 {
                scan_idx += 1;
                next_scan_offset = if scan_idx + 1 < scan_offsets.len() {
            ...
            if let Some((start, end)) = scan_filter {
                if scan_idx < start || scan_idx > end { continue; }
            }
            
            let tof = tof_indices[p_idx];
            let intensity = intensities[p_idx];
            
            let mz = mz_cv.convert(tof as f64) as f32;
            let im = im_cv.convert(scan_idx as f64) as f32;
            ...
            data.scan_indices.push(scan_idx as u32);
        }
    }
...
// ============================================================================
fn merge_chunks_parallel(chunks: Vec<TimsTOFData>) -> TimsTOFData {
    if chunks.is_empty() {
        return TimsTOFData::new();
    }
    
    let chunk_size = (chunks.len() / get_num_threads()).max(1);
    
    chunks
        .into_par_iter()
        .chunks(chunk_size)
        .map(|chunk_group| {
            let total_cap: usize = chunk_group.iter().map(|c| c.mz_values.len()).sum();
            let mut merged = TimsTOFData::with_capacity(total_cap);
            for mut chunk in chunk_group {
                unsafe { merged.append_unchecked(&mut chunk); }
            }
            merged
        })
        .reduce(
            || TimsTOFData::new(),
            |mut acc, mut chunk| {
                unsafe { acc.append_unchecked(&mut chunk); }
                acc
            }
        )
}
...
// Fast estimation
// ============================================================================
fn estimate_total_peaks_fast(frames: &FrameReader) -> Result<(usize, usize), Box<dyn Error>> {
    let sample_size = std::cmp::min(100, frames.len());
    let mut ms1_peaks = Vec::with_capacity(sample_size);
    let mut ms2_peaks = Vec::with_capacity(sample_size);
    
    let step = frames.len() / sample_size;
    
    for i in 0..sample_size {
        let idx = i * step;
        if let Ok(frame) = frames.get(idx) {
            match frame.ms_level {
                MSLevel::MS1 => ms1_peaks.push(frame.tof_indices.len()),
                MSLevel::MS2 => ms2_peaks.push(frame.tof_indices.len()),
                _ => {}
            }
        }
    }
    
    ms1_peaks.sort_unstable();
    ms2_peaks.sort_unstable();
    
    let ms1_median = if !ms1_peaks.is_empty() {
        ms1_peaks[ms1_peaks.len() / 2]
    } else {
        10000
    };
    
    let ms2_median = if !ms2_peaks.is_empty() {
        ms2_peaks[ms2_peaks.len() / 2]
    } else {
        10000
    };
    
    let total_frames = frames.len();
    let estimated_ms1_frames = (total_frames * 2) / 5;
    let estimated_ms2_frames = (total_frames * 3) / 5;
    
    let estimated_ms1 = ms1_median * estimated_ms1_frames;
    let estimated_ms2 = ms2_median * estimated_ms2_frames;
    
    Ok((estimated_ms1, estimated_ms2))
}

// ============================================================================
// OPTIMIZED read_timstof_data with parallel aggregation
// ============================================================================
pub fn read_timstof_data(d_folder: &Path) -> Result<TimsTOFRawData, Box<dyn Error>> {
    ...
    let (ms1_estimate, ms2_estimate) = estimate_total_peaks_fast(&frames)?;
    println!("  Estimated MS1 peaks: ~{}", ms1_estimate);
    println!("  Estimated MS2 peaks: ~{}", ms2_estimate);
    
    let num_threads = get_num_threads();
    println!("Processing frames with optimized pipeline ({} threads)...", num_threads);
    let process_start = Instant::now();
    
    let ms1_chunks = Arc::new(RwLock::new(Vec::<TimsTOFData>::with_capacity(n_frames / 100)));
    let ms2_map = Arc::new(DashMap::with_capacity_and_hasher_and_shard_amount(
        200,
    ...
    frame_indices.par_chunks(BATCH_SIZE).for_each_with(
        (Arc::clone(&frames), Arc::clone(&mz_cv), Arc::clone(&im_cv), 
         Arc::clone(&ms1_chunks), Arc::clone(&ms2_map), Arc::clone(&processed_count)),
        |(frames, mz_cv, im_cv, ms1_chunks, ms2_map, processed_count), chunk| {
            
            let mut local_ms1 = TimsTOFData::with_capacity(MS1_CHUNK_SIZE);
            let mut local_ms2: HashMap<(u32, u32), TimsTOFData, ahash::RandomState> = 
                HashMap::with_capacity_and_hasher(32, ahash::RandomState::new());
            ...
            for &idx in chunk {
                ...
                match frame.ms_level {
                    MSLevel::MS1 => {
                        let ms1 = process_peaks_vectorized_optimized(
                            &frame.tof_indices,
                            &frame.intensities,
                            &frame.scan_offsets,
                            rt_min,
                        ...
                        if !ms1.mz_values.is_empty() {
                            local_ms1.merge_from(ms1);
                            
                            if local_ms1.mz_values.len() >= MS1_CHUNK_SIZE {
                                let chunk = std::mem::replace(&mut local_ms1, TimsTOFData::with_capacity(MS1_CHUNK_SIZE));
                                ms1_chunks.write().push(chunk);
                            }
                        }
                    }
                    MSLevel::MS2 => {
                        ...
                        for win in 0..qs.isolation_mz.len() {
                            ...
                            let td = process_peaks_vectorized_optimized(
                                &frame.tof_indices,
                                &frame.intensities,
                                &frame.scan_offsets,
                                rt_min,
                            ...
                            if !td.mz_values.is_empty() {
                                local_ms2.entry(key)
                                    .or_insert_with(|| TimsTOFData::with_capacity(10000))
                                    .merge_from(td);
                            }
                        }
                    }
            ...
            if !local_ms1.mz_values.is_empty() {
                ms1_chunks.write().push(local_ms1);
            }
            
            for (key, data) in local_ms2 {
                ms2_map.entry(key)
                    .or_insert_with(|| Arc::new(Mutex::new(TimsTOFData::with_capacity(100000))))
                    .lock()
                    .merge_from(data);
            }
        }
    ...
    println!("Finalizing data structures with parallel merge...");
    let finalize_start = Instant::now();
    
    let ms1_chunks = Arc::try_unwrap(ms1_chunks).unwrap().into_inner();
    let global_ms1 = merge_chunks_parallel(ms1_chunks);
    
    // First unwrap the Arc around ms2_map
    let ms2_map = Arc::try_unwrap(ms2_map)
        .expect("Failed to unwrap Arc<DashMap>");
    
    let ms2_vec: Vec<_> = ms2_map
        .into_iter()
        .par_bridge()
        .map(|(key, data)| {
            let q_low = key.0;
...
// ============================================================================
// OPTIMIZED IndexedTimsTOFData with radix sort
// ============================================================================
...
fn radix_argsort(values: &[u32]) -> Vec<usize> {
    let n = values.len();
    ...
    let num_bits = 32 - max_val.leading_zeros();
    let num_passes = (num_bits + 7) / 8;
...
fn parallel_radix_argsort(values: &[u32]) -> Vec<usize> {
    let n = values.len();
    ...
    for pass in 0..num_passes {
        ...
        let thread_counts: Vec<Vec<usize>> = indices
            .par_chunks(n / rayon::current_num_threads().max(1))
            .map(|chunk| {
                let mut local_counts = vec![0usize; 256];
                for &idx in chunk {
                    let byte = ((values[idx] >> shift) & 0xFF) as usize;
                    local_counts[byte] += 1;
                }
        ...
        let mut cumulative = 0;
        for count in &mut global_counts {
            let temp = *count;
            *count = cumulative;
            cumulative += temp;
        }
        
        let offsets = global_counts.clone();
        
        for &idx in &indices {
            let byte = ((values[idx] >> shift) & 0xFF) as usize;
            temp_indices[offsets[byte]] = idx;
        }
...
impl IndexedTimsTOFData {
    ...
    pub fn from_timstof_data(data: TimsTOFData) -> Self {
        ...
        if n_peaks < PARALLEL_THRESHOLD {
            Self::from_timstof_data_sequential_radix(data)
        } else {
            Self::from_timstof_data_parallel_radix(data)
        }
    }
    
    fn from_timstof_data_sequential_radix(data: TimsTOFData) -> Self {
        let n_peaks = data.mz_values.len();
        
        let mz_ints: Vec<u32> = data.mz_values.iter()
            .map(|&mz| unsafe { (mz * 1_000_000.0).to_int_unchecked::<u32>() })
            .collect();
        
        let order = radix_argsort(&mz_ints);
        
        let mut result = Self {
            rt_values_min: vec![0.0; n_peaks],
            mobility_values: vec![0.0; n_peaks],
            mz_values: vec![0.0; n_peaks],
            intensity_values: vec![0; n_peaks],
            frame_indices: vec![0; n_peaks],
            scan_indices: vec![0; n_peaks],
        };
        
        for (new_idx, &old_idx) in order.iter().enumerate() {
            result.rt_values_min[new_idx] = data.rt_values_min[old_idx];
            result.mobility_values[new_idx] = data.mobility_values[old_idx];
            result.mz_values[new_idx] = data.mz_values[old_idx];
            result.intensity_values[new_idx] = data.intensity_values[old_idx];
            result.frame_indices[new_idx] = data.frame_indices[old_idx];
            result.scan_indices[new_idx] = data.scan_indices[old_idx];
        }
        
        result
    }
    
    fn from_timstof_data_parallel_radix(data: TimsTOFData) -> Self {
        let n_peaks = data.mz_values.len();
        ...
        let order = parallel_radix_argsort(&mz_ints);
        ...
        result.rt_values_min.par_chunks_mut(CHUNK_SIZE)
            .zip(result.mobility_values.par_chunks_mut(CHUNK_SIZE))
            .zip(result.mz_values.par_chunks_mut(CHUNK_SIZE))
            .zip(result.intensity_values.par_chunks_mut(CHUNK_SIZE))
            .zip(result.frame_indices.par_chunks_mut(CHUNK_SIZE))
            .zip(result.scan_indices.par_chunks_mut(CHUNK_SIZE))
            .enumerate()
            .for_each(|(chunk_idx, (((((rt, mob), mz), int), frame), scan))| {
                let base_idx = chunk_idx * CHUNK_SIZE;
                let chunk_end = (base_idx + CHUNK_SIZE).min(n_peaks);
                
                for i in base_idx..chunk_end {
                    let old_idx = order[i];
                    let local_i = i - base_idx;
                    rt[local_i] = data.rt_values_min[old_idx];
                    mob[local_i] = data.mobility_values[old_idx];
                    mz[local_i] = data.mz_values[old_idx];
                    int[local_i] = data.intensity_values[old_idx];
                    frame[local_i] = data.frame_indices[old_idx];
                    scan[local_i] = data.scan_indices[old_idx];
                }
            });
    ...
    fn range_indices(&self, mz_min: f32, mz_max: f32) -> std::ops::Range<usize> {
        let mz_min_int = unsafe { (mz_min * 1_000_000.0).to_int_unchecked::<u32>() };
        let mz_max_int = unsafe { (mz_max * 1_000_000.0).to_int_unchecked::<u32>() };
        
        let start = self.binary_search_left(mz_min_int);
        let end = self.binary_search_right(mz_max_int);
        
        start..end
    }
    
    #[inline(always)]
    fn binary_search_left(&self, target: u32) -> usize {
        let mut left = 0;
        let mut right = self.mz_values.len();
        
        while left < right {
            let mid = left + (right - left) / 2;
            let mid_val = unsafe { (self.mz_values[mid] * 1_000_000.0).to_int_unchecked::<u32>() };
    ...
    #[inline(always)]
    fn binary_search_right(&self, target: u32) -> usize {
        let mut left = 0;
        let mut right = self.mz_values.len();
    ...
    pub fn slice_by_mz_range(&self, mz_min: f32, mz_max: f32) -> TimsTOFData {
        ...
        unsafe {
            td.rt_values_min.set_len(cap);
            td.mobility_values.set_len(cap);
            td.mz_values.set_len(cap);
            td.intensity_values.set_len(cap);
            td.frame_indices.set_len(cap);
            td.scan_indices.set_len(cap);
            
            ptr::copy_nonoverlapping(
                self.rt_values_min[range.start..].as_ptr(),
                td.rt_values_min.as_mut_ptr(),
                cap
            );
            ptr::copy_nonoverlapping(
                self.mobility_values[range.start..].as_ptr(),
                td.mobility_values.as_mut_ptr(),
                cap
            );
            ptr::copy_nonoverlapping(
                self.mz_values[range.start..].as_ptr(),
                td.mz_values.as_mut_ptr(),
                cap
            );
            ptr::copy_nonoverlapping(
                self.intensity_values[range.start..].as_ptr(),
                td.intensity_values.as_mut_ptr(),
                cap
            );
            ptr::copy_nonoverlapping(
                self.frame_indices[range.start..].as_ptr(),
                td.frame_indices.as_mut_ptr(),
                cap
            );
            ptr::copy_nonoverlapping(
                self.scan_indices[range.start..].as_ptr(),
                td.scan_indices.as_mut_ptr(),
                cap
            );
        }
    ...
    pub fn slice_by_mz_im_range(&self, mz_min: f32, mz_max: f32, im_min: f32, im_max: f32) -> TimsTOFData {
        ...
        let indices: Vec<usize> = if range.len() > PARALLEL_THRESHOLD {
    ...
    pub fn convert_mz_to_integer(&mut self) {
        self.mz_values.par_iter_mut()
            .for_each(|v| *v = (*v * 1000.0).ceil());
    }
...
pub fn build_indexed_data(raw_data: TimsTOFRawData) -> Result<(IndexedTimsTOFData, Vec<((f32, f32), IndexedTimsTOFData)>), Box<dyn Error>> {
    ...
    let (ms1_indexed, ms2_indexed_pairs) = rayon::join(
        || {
            println!("  Building MS1 index with radix sort...");
            IndexedTimsTOFData::from_timstof_data(raw_data.ms1_data)
        },
        || {
            println!("  Building MS2 indices with radix sort...");
    ...
    println!("  Index building completed in {:.3}s", build_start.elapsed().as_secs_f32());
...
impl MergeFrom for TimsTOFData {
    ...
    fn merge_from(&mut self, mut other: Self) {
        unsafe { self.append_unchecked(&mut other); }
    }
}

// ============================================================================
// Library processing functions - keeping original implementation
// ============================================================================

...
fn main() -> Result<(), Box<dyn Error>> {
    ...
    let parallel_threads = if let Ok(slurm_cpus) = env::var("SLURM_CPUS_PER_TASK") {
        slurm_cpus.parse::<usize>().unwrap_or_else(|_| {
    ...
    let (default_data_folder, lib_file_path, report_file_path) = if is_macos {
        ...
        (
            ...
            "/wangshuaiyao/dia-bert-timstof/lib/TPHPlib_frag1025_swissprot_final_all_from_Yueliang_with_decoy.tsv"
